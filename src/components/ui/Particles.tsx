...
export const Particles = ({
  ...
  const [color, setColor] = useState("rgba(100, 255, 218,"); // default teal
  const [refreshSignal, setRefreshSignal] = useState(false);

  useEffect(() => {
    // Update color based on theme
    if (resolvedTheme === "light") {
      setColor("rgba(0, 0, 0,"); // Pure black for maximum contrast in light mode
    } else {
      setColor("rgba(100, 255, 218,"); // Bright teal for dark mode
    }
  ...
  const drawCircle = (circle: Circle, update = false) => {
    if (context.current) {
      ...
      context.current.fillStyle = `${color} ${alpha})`; // Dynamic color
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      else if (circle.y < -circle.size) circle.y = canvasSize.current.h + circle.size;
  ...
  return (
    <div
      ...
      aria-hidden="true"
      onMouseMove={(e) => {
        // Simple mouse interaction tracking relative to center
